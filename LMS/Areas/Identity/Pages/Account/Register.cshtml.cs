...
namespace LMS.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        {
            var studentIdQuery = from s in db.Students
                                  orderby s.UId descending
                                  select s.UId;

            var professorIdQuery = from s in db.Professors
                                   orderby s.UId descending
                                   select s.UId;

            var adminIdQuery = from s in db.Administrators
                                   orderby s.UId descending
                                   select s.UId;

            string firstStudentID = studentIdQuery.FirstOrDefault();
            string firstProfessorID = professorIdQuery.FirstOrDefault();
            string firstAdminID = adminIdQuery.FirstOrDefault();
            ...
            {
                if (id != null && int.Parse(id.Substring(1)) > nextUID)
                    nextUID = int.Parse(id.Substring(1));
            }
            ...
            else if (role == "Professor")
            {
                ...
                professor.Department = departmentAbbrev;
                professor.UId = "u" + nextUID;
